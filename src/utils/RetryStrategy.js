class RetryStrategy {
  constructor(options = {}) {
    ...
    this.maxRetries = options.maxRetries || 10; // Increased retries
  ...
  calculateDelay(attempt, errorType) {
    ...
    if (errorType === 'complexity') {
      // Longer delays for complexity errors
      baseDelay = Math.min(this.baseDelay * 4 * attempt, this.maxDelay);
    } else {
      baseDelay = Math.min(Math.pow(2, attempt) * this.baseDelay, this.maxDelay);
    }

    // Add more jitter for complexity errors
    const jitterFactor = errorType === 'complexity' ? this.jitterFactor * 2 : this.jitterFactor;
